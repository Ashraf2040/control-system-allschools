...
const ClassSubjectsManagement = () => {
    ...
    if (selectedClass && selectedSubjects.length > 0) {
      try {
        const res = await fetch('/api/classSubjects', {
          method: 'POST',
          body: JSON.stringify({
            type: 'assignSubjects', // Specify the type as 'assignSubjects'
    ...
    if (selectedClass && selectedTeachers.length > 0) {
      try {
        const res = await fetch('/api/classSubjects', {
          method: 'POST',
          body: JSON.stringify({
            type: 'assignTeacher', // Specify the type as 'assignTeacher'
            classId: selectedClass,
  ...
  const handleAddClass = async () => {
    try {
      const res = await fetch('/api/classSubjects', {
        method: 'POST',
        body: JSON.stringify({
          type: 'class', // Specify the type as 'class'
          name: newClassName,
        }),
        headers: {
          'Content-Type': 'application/json',
  ...
  const handleAddSubject = async () => {
    if (newSubjectName.name.trim() !== '' && newSubjectName.arabicName.trim() !== '') {
      const res = await fetch('/api/classSubjects', {
        method: 'POST',
        body: JSON.stringify({
          type: 'subject', // Specify the type as 'subject'
  ...
  // Function to generate marks for the newly assigned subjects to the class
